...
export default function Sizes({ sizes, product, setProduct }) {
    ...
    return (
        <div>
            <div className={styles.header}>Quantity / Price</div>
            ...
            </button> */}
            {
                sizes ? sizes.map((size, i) => (
                    <div className={styles.clicktoadd} key={i}>
                        <select name="size" value={noSize ? "" : size.size} disabled={noSize} style={{ display: `${noSize ? "none" : ""}` }} onChange={(e) => handleSize(i, e)}>
                        ...
                        <input type="number" name="qty"
                            placeholder={noSize ? "Product Quantity" : "Sizes Quantity"}
                            min={1}
                            value={size.qty}
                            onChange={(e) => handleSize(i, e)}
                        />
                        <input type="number" name="price"
                            placeholder={noSize ? "Product Price" : "Sizes Price"}
                            min={1}
                            value={size.price}
                            onChange={(e) => handleSize(i, e)}
                        />
